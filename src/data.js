...
export const materials=Object.values({...profiles,...extended,...brands}).map((m,i)=>({...m,index:i+1,history:m.history||histories[m.id],aliases:aliases[m.id]||'',color:['#a6b49e','#c8b89d','#b6c0ae','#aeacb1','#c0a18d','#a4b6b3'][i%6]}));
...
export function materialSearch(m,q){return normalize([m.name,m.id,m.family,m.aliases,m.composition,m.uses].join(' ')).includes(normalize(q));}
...
export function getMaterials({q='',family='All materials',sort='featured',mapped=false}={}){
 let list=materials.filter(m=>materialSearch(m,q)&&(family==='All materials'||m.family===family)&&(!mapped||networks[m.id]));
