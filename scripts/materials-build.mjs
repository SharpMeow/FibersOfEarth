...
export async function buildMaterials(cssFile,base){
 ...
 for(const m of materials){
  ...
  const dsRefs=m.data?.refs||[],src=i=>dsRefs[i]?` <a class="src-link" href="${e(dsRefs[i].url)}" rel="noopener noreferrer" title="${e(dsRefs[i].title)}">[${i+1}]</a>`:'';
  const dataSheet=m.data?.properties?.length?`<h3>Data sheet</h3><div class="table-wrap"><table class="data-sheet"><thead><tr><th scope="col">Property</th><th scope="col">Value</th><th scope="col">Conditions</th><th scope="col">Source</th></tr></thead><tbody>${m.data.properties.map(x=>`<tr><th scope="row">${e(x.label)}</th><td>${e(x.value)}${x.unit?' '+e(x.unit):''}</td><td>${e(x.conditions)}</td><td>${src(x.ref)}</td></tr>`).join('')}</tbody></table></div>`:'';
  const timeline=m.data?.timeline?.length?`<h3>Timeline</h3><ol class="timeline">${m.data.timeline.map(x=>`<li><span class="tl-year">${e(x.year)}</span><span class="tl-event">${e(x.event)}${src(x.ref)}</span></li>`).join('')}</ol>`:'';
  const production=m.data?.production?`<p class="notice"><strong>Production:</strong> ${e(m.data.production.figure)} (${e(m.data.production.scope||'')}${m.data.production.year?', '+e(m.data.production.year):''}).${m.data.production.countries?' Main producers: '+e(m.data.production.countries)+'.':''}${src(m.data.production.ref)}</p>`:'';
  const refs=[...m.sources.map(id=>sources.find(s=>s.id===id)).filter(Boolean).map(s=>({title:s.name+': '+s.title,url:s.url})),...(d.refs||[]),...(g?.refs||[]),...dsRefs].filter((r,i,a)=>a.findIndex(x=>x.url===r.url)===i);
...
${sec('science','Science',`${dataSheet}<h3>${e(sci[0])}</h3>${p(sci[1])}${d.structure?`<h3>Structure and chemistry</h3>${p(d.structure)}`:''}${d.process?`<h3>From source to yarn</h3>${p(d.process)}`:''}${d.performance?`<h3>Performance in use</h3>${p(d.performance)}`:''}${d.identify?`<h3>How it is identified</h3>${p(d.identify)}`:''}`)}
${sec('history','History',`${timeline}${p(m.history)}${d.past?`<h3>The deeper record</h3>${p(d.past)}`:''}`)}
